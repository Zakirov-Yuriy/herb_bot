...
def mode_calib():
    with sync_playwright() as p:
        ...
        print("\n---------------------------------------------------------------------------")
        print(">>> 1.2 СЕЙЧАС ВЫБИРАЕМ РЕСУРСЫ, КОТОРЫЕ БУДЕМ СОБИРАТЬ.")
        print("---------------------------------------------------------------------------")
        print(">>> Это «пипетка»: ты КЛИКАЕШЬ прямо по ресурсу в игре, а бот запоминает")
        print(">>> его цвет и потом ищет и собирает именно такие ресурсы.")
        print(">>> Так надёжнее всего (особенно для камней/рыбы разного цвета).")
        print(">>> ")
        print(">>> КАК ДЕЛАТЬ:")
        print(">>>   1) реши, сколько образцов цвета снять (для камней РАЗНОГО цвета -")
        print(">>>      по одному образцу на каждый цвет);")
        print(">>>   2) кликай ОДИН раз ровно по ЦЕНТРУ ресурса, затем ENTER.")
        print(">>> Не хочешь снимать цвет вручную? Введи 0 - бот возьмёт готовый цвет")
        print(">>> для этой профессии.")
        ans = read_line_keep_alive(
        ...
        print("\n---------------------------------------------------------------------------")
        print(">>> 1.3 СЕЙЧАС ВЫБИРАЕМ РЕСУРСЫ, КОТОРЫЕ БУДЕМ ИГНОРИРОВАТЬ (по желанию).")
        print("---------------------------------------------------------------------------")
        print(">>> Это обратная «пипетка»: ты кликаешь по тому, что бот трогать НЕ должен,")
        print(">>> и эти цвета попадают в ЧЁРНЫЙ СПИСОК - по ним бот кликать не будет.")
        print(">>> ")
        print(">>> КОГДА НУЖНО: если бот по ошибке тыкает по ТРАВЕ, ФОНУ или ЧУЖИМ ресурсам")
        print(">>> - кликни прямо по НИМ здесь.")
        print(">>> ")
        print(">>> ⚠ ВАЖНО: НЕ кликай по ресурсу, который СОБИРАЕШЬ! Если занести его цвет")
        print(">>>   в игнор - бот перестанет его собирать. (Бот теперь это ловит и не даст")
        print(">>>   добавить цвет ресурса, но лучше просто кликай по фону/траве.)")
        print(">>> Если бот вообще НЕ собирает ресурс - введи 'c', чтобы ОЧИСТИТЬ игнор.")
        print(">>> Если пока всё и так нормально - смело вводи 0 и пропусти этот шаг.")
