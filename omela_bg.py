"""
omela_bg.py — фоновый автосбор ОМЕЛЫ + авто-бой для браузерной игры dwar / Легенда.

Что умеет:
  • Сбор омелы кликом внутри страницы (окно можно свернуть). Добыча запускается
...
Режимы:
  ...
  python omela_bg.py --calib   # МАСТЕР КАЛИБРОВКИ поэтапно: карта, добыча, «закрыть», бой
...
def apply_saved_config():
    ...
    log.info("Добыча: %d клик(а/ов) на куст.", GATHER_CLICKS)
...
def mode_debug():
    with sync_playwright() as p:
        ...
        log.info("Найдено 'омелы': %d. Файлы page_full_%s.png / page_dom_%s.html.",
                 len(find_omela(crop_map(full))), stamp, stamp)
...
def mode_calib():
    with sync_playwright() as p:
        ...
        print(">>> Кликни по ЧУЖОМУ ресурсу (рыба/руда), чтобы вылезло окно «нет профессии».")
...
def gather_visible(page, scroll_pos, total):
    ...
    if len(pts) > MAX_PER_CYCLE:
        ...
        return total, False
    if pts:
        log.info("Вижу омелы: %d шт. (прокрутка %d).", len(pts), scroll_pos)
    for (cx, cy) in pts:
...
def main():
    ap = argparse.ArgumentParser(description="Автосбор омелы + авто-бой (Playwright).")
    ap.add_argument("--login", action="store_true", help="только войти (сохранить сессию)")
    ap.add_argument("--calib", action="store_true", help="мастер калибровки (карта/добыча/закрыть/бой)")
